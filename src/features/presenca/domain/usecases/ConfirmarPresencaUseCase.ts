...
export interface IPresencaRepository {
  confirmarPresenca(escalaId: string, membroId: string, confirmar: boolean, justificativa?: string): Promise<void>;
}

export class ConfirmarPresencaUseCase {
  constructor(private readonly presencaRepository: IPresencaRepository) {}

  async execute(escalaId: string, membroId: string, confirmar: boolean, justificativa?: string): Promise<void> {
    ...
    await this.presencaRepository.confirmarPresenca(escalaId, membroId, confirmar, justificativa);
  }
}
